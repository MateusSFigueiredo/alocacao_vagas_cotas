...
# Preenche para um centro, todos os anos
for (i in 2013:2022){
dados_ufv[cursos_ccb[cursos_ccb!="MEDICINA"]][Processo_Seletivo == paste0("SISU",i)] %>%
  nrow()
}

for (i in 2013:2022){
dados_ufv[Curso %in% cursos_ccb[cursos_ccb!="MEDICINA"]][Processo_Seletivo == paste0("SISU",i)] %>%
  nrow() %>% print()
}          
